class DeadZone_RepInfo extends ReplicationInfo;

const DeadZone_LMT = class'DeadZone_LocalMessage';

enum DeadZone_PlayerType
{
	DeadZone_Unknown,
	DeadZone_Player,
	DeadZone_Group,
...
var private IDeadZone DeadZone;
...
public simulated function DeadZone_PlayerType PlayerType()
{
	`Log_Trace();
	
	if (IsServerOwner())
	{
		return DeadZone_Owner;
	}
		
	if (IsAdmin())
	{
		return DeadZone_Admin;
	}
		
	if (IsGroupMember())
	{
		return DeadZone_Group;
	}
		
	return DeadZone_Player;
}
...
private reliable server function ServerApplyMembership()
{
	...
	if (PlayerType() <= DeadZone_Group)
	{
		`Log_Info("Increase boost:" @ PlayerType());
		DeadZone.IncreaseXPBoost(GetKFPC());
	}
}

public reliable client function WriteToChatLocalized(E_DeadZone_LocalMessageType LMT, String HexColor, optional String String1, optional String String2, optional String String3)
{
	`Log_Trace();
	
	WriteToChat(DeadZone_LMT.static.GetLocalizedString(LogLevel, LMT, String1, String2, String3), HexColor);
}
...
public reliable client function WriteToHUDLocalized(E_DeadZone_LocalMessageType LMT, optional String String1, optional String String2, optional String String3, optional float DisplayTime = 0.0f)
{
	`Log_Trace();
	
	WriteToHUD(DeadZone_LMT.static.GetLocalizedString(LogLevel, LMT, String1, String2, String3), DisplayTime);
}
